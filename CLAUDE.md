...
## No test suite
There are no automated tests in this repo — verify changes by running the server and hitting endpoints manually (`GET /health`, `POST /chat`, etc.).

## Architecture (5 files, all in `app/`)
...
- `embeddings.py` — VoyageAI; `embed_documents()` uses `input_type="document"`, `embed_query()` uses `input_type="query"` (asymmetric — don't mix them up)
...
- `rag.py` — retrieval (cosine distance via `<=>`) + Claude call
...
- All `/ingest/*` and `/documents*` endpoints require header `x-api-key: <INGEST_API_KEY>`.
- CORS is wide open (`allow_origins=["*"]`) in `app/main.py` — intentional for dev only; must be locked down before real deployment.
